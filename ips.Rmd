---
title: "Costa Rica: IPS Cantonal"
output: 
  flexdashboard::flex_dashboard:
    orientation: columns
    vertical_layout: fill
runtime: shiny
---

```{r setup, include=FALSE}
library(flexdashboard)
require(tidyr)
require(dplyr)
require(ggplot2)
require(plotly)

ips <- read.csv("./inputs/cr-IPS-cantonal.csv", stringsAsFactors=FALSE)
indices <- names(ips)[4:dim(ips)[2]]
indices <- gsub("\\."," ",indices)
```

Indices - Box Plots
=======================================================================

Inputs {.sidebar}
-----------------------------------------------------------------------

```{r}
selectInput("indice", label = h3("Indice"),
    choices = indices,
    selected = 1)
```

Se observa el comportamiento de cada indicador por provincia.

Row
-----------------------------------------------------------------------

### Box Plot

```{r}
indi <- reactive({gsub(" ","\\.",input$indice)})

tmp <- reactive({
  tmp0 <- ips[,c(indi(),"Provincia","Cantón")]
  names(tmp0)[1] <- "Indice"
  tmp0
  })

renderPlotly({
  plot_ly(tmp(), y = Indice, color = as.factor(Provincia), type = "box")
})
```























Indices - Scatter Plots
=======================================================================

Inputs {.sidebar}
-----------------------------------------------------------------------

```{r}
selectInput("enX", label = h3("Variable en eje x:"),
    choices = indices,
    selected = 1)

selectInput("enY", label = h3("Variable en eje y:"),
    choices = indices,
    selected = 2)
```

Se observa el comportamiento de la relación entre dos indicadores.

Row
-----------------------------------------------------------------------

### Box Plot

```{r}
enX_call <- reactive(gsub(" ","\\.",input$enX))
enY_call <- reactive(gsub(" ","\\.",input$enY))
# tmp <- reactive({ts(clicksMovil[,produ()],frequency = 28) %>% decompose()})
# renderPlotly({plot_ly(x = clicksMovil$Day.of.Month, y = tmp()$x, name="Observado") %>%
#     layout(title = paste("Eventos de",input$producto),
#            xaxis = list(title="Fecha"), yaxis = list(title="Eventos"))})

renderPlotly({
  # plot1 <- ggplot(data = ips, aes(x = as.call(enX_call()), y = as.call(enY_call()))) +
  plot1 <- ggplot(data = ips, aes(x = ips[,enX_call()], y = ips[,enY_call()])) +
    geom_point(aes(colour=Provincia))
  ggplotly(plot1)
  })
```
